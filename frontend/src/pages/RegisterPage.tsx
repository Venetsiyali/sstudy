                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Sardor Karimov" required
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Kamida 6 ta belgi" required
                            ...
                            {password.length > 0 && (
                                <div className="flex gap-1 mt-2">
                                    {[1, 2, 3, 4].map(i => (
                                        <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${passwordStrength >= i ? i <= 1 ? "bg-red-400" : i <= 2 ? "bg-amber-400" : "bg-emerald-400" : "bg-slate-200"}`} />
                                    ))}
                                </div>
                            )}
                        </div>
                        <div>
                            ...
                            <div className="relative">
                                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input type={showPass ? "text" : "password"} value={confirmPass} onChange={e => setConfirmPass(e.target.value)} placeholder="Parolni qayta kiriting" required
                                    className={`w-full pl-11 pr-11 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all ${confirmPass.length > 0 ? passwordsMatch ? "border-emerald-300 bg-emerald-50/50" : "border-red-300 bg-red-50/50" : "border-slate-200"}`} />
                                {passwordsMatch && <Check className="absolute right-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-emerald-500" />}
                            </div>
                        </div>
                        <button type="submit" disabled={loading || !passwordsMatch}
                            ...
                            {loading ? <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <>Ro'yxatdan o'tish <ArrowRight className="w-4 h-4" /></>}
                        </button>
                    </form>
                    <div className="mt-6 text-center">
                        <p className="text-sm text-slate-500">Allaqachon hisobingiz bormi? <Link to="/login" className="text-indigo-600 hover:text-indigo-700 font-semibold">Kirish</Link></p>
                    </div>
                </div>
