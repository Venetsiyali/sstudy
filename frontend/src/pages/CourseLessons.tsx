...
import { getCourseWithLessons, type Course, type Lesson } from "@/lib/supabase";
...
export function CourseLessons() {
    ...
    const loadCourse = async () => {
        setLoading(true);
        const data = await getCourseWithLessons(Number(id));
        ...
        <div className="max-w-4xl mx-auto space-y-8">
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="sm" asChild>
                    <Link to="/" className="gap-2"><ArrowLeft className="w-4 h-4" /> Orqaga</Link>
                </Button>
            </div>
            ...
            <div className="space-y-3">
                <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <BookOpen className="w-5 h-5 text-indigo-600" /> Darslar ro'yxati
                </h2>
                {lessons.length === 0 ? (
                    <div className="py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                        <p className="text-slate-400">Hozircha darslar yo'q</p>
                    </div>
                ) : (
                    lessons.map((lesson, i) => (
                        <Link key={lesson.id} to={`/lesson/${lesson.id}?course=${course.id}`}
                            ...
                            <div className="relative w-32 h-20 rounded-xl overflow-hidden shrink-0 bg-slate-900">
                                <img src={lesson.thumbnail} alt={lesson.title} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity"
                                    onError={e => { (e.target as HTMLImageElement).src = `https://i.ytimg.com/vi/${lesson.video_id}/hqdefault.jpg`; }} />
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">#{lesson.lesson_order}-dars</span>
                                </div>
                                <h3 className="font-semibold text-slate-900 group-hover:text-indigo-700 transition-colors truncate">{lesson.title}</h3>
